...
// Function to fetch all reviews associated with a provider using the provider ID.
const getReviewsByProviderId = async (req, res) => {
  const { providerID } = req.params;

  if (!providerID) {
    return res.status(400).json(new ApiError(400, "Provider ID is required"));
  }

  try {
    const reviews = await Review.findAll({ where: { providerID } });
...
export { addReview, deleteProvider, getReviewsByProviderId };