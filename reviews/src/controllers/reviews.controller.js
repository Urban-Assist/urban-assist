...
const USER_MGMT_URL = process.env.USER_MGMT_URL || 'http://localhost:8083';
...
const getProviderReviews = async (req, res) => {
  ...
  try {
    ...
    console.error(error);
    res.status(400).send({ message: "There was an error fetching reviews" });
  }
};
...
const addReview = async (req, res) => {
  ...
  try {
    ...
    // Update provider's stars in userManagement service
    try {
      await axios.put(`${USER_MGMT_URL}/api/providers/${review.providerID}/stars`, {
        stars: Math.round(averageRating * 10) / 10 // Round to 1 decimal place
      });
    } catch (updateError) {
      console.error('Failed to update provider stars:', updateError);
      // Don't fail the review creation if provider update fails
    }
...
export { addReview, deleteProvider, getProviderReviews };